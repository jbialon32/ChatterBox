<?php
  ...
  if(isset($_POST["chat_id"])&&isset($_POST["timestampindex"])){
    
      $chatID = $_POST["chat_id"];
      $timeStampIndex = $_POST["timestampindex"];
      //Query to fetch message details
      $query = "SELECT chat_id, user_name, message, time_stamp FROM message_full WHERE chat_id=? AND
                                                                         time_stamp>?";
      if($stmt = $db->prepare($query)){
        //Bind chat_id parameter to the query
        $stmt->bind_param('ss', $chatID, $timeStampIndex);
        $stmt->execute();
